...
Step 3: Create a function called getTodos that will fetch the API using axios if the length of the array in the
localstorage is 0. What this means is that, on page load, if the localstorage is empty, it will fetch the API
to get the "todos" resources from it, and if there are no errors, the response of the "get" method used will call
the createList function previously defined with the response date in its parameter. Call this function (getTodos).
Step 4: Call the displayOnPage function, that will display the values stored in the localstorage inside of the 
<ul>.
Step 5: Create a function called addToList that creates a set of elements on which the todos submitted on the input 
...
Step 6: Create a function called saveTodosLocalstorage that takes in a todo as a parameter. This todo refers to the input value that was submitted
...
method.
Step 7: Create a function called displayOnPage, that firstly defines a variable called "list" with the retrieval of the values in the localstorage as an array.
...
Step 8: Create a function called deleteChecked that removes from the UI and the local storage the selected list items on the UI. The list items are selected by
